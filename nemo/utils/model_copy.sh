...
if [ "$RANK" -eq 0 ]; then
    ...
    mkdir -p "$(dirname "$COPY_COMPLETE_PATH")"  # Ensure the directory exists
  ...
  while [ ! -f "$COPY_COMPLETE_PATH" ]; then
    sleep 5  # Wait for 1 second before checking again
  done

  echo "$COPY_COMPLETE_PATH found on RANK=$RANK. Resuming the opeartion."
